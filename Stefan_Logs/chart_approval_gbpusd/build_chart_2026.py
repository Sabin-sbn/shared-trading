#!/usr/bin/env python3
"""Construieste _charts/gbpusd_draw_2026.html — Mini-TradingView local cu date GBPUSD.pro M15 (2025-2026).
Generat automat din template-ul principal (build_chart_html.py) cu adaptari pentru 2026.
Retrace markers + sidebar collapse + toate feature-urile din chart-ul principal."""
import csv, os, json

HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(HERE, "gbpusd_pro_m15_2025_2026.csv")
SRC_M1 = None   # 2026 chart nu are M1 (prea multe date)
OUT = os.path.join(HERE, "gbpusd_draw_2026.html")
...
rows_m1 = []
if SRC_M1 and os.path.exists(SRC_M1):
...
# 2026: incarc cele 425 trade-uri retrace din JSON
RETRACE_PATH = os.path.join(HERE, "gbpusd_2026_retrace.json")
if os.path.exists(RETRACE_PATH):
    with open(RETRACE_PATH, encoding="utf-8") as f:
        retrace_markers = json.load(f)
else:
    retrace_markers = []
retrace_js = json.dumps(retrace_markers, separators=(",", ":"), ensure_ascii=False)
...
<head>
<meta charset="UTF-8">
<title>Mini-TradingView — GBPUSD.pro M15 (2025-2026)</title>
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .mk .adc { display:flex; gap:4px; margin-top:4px; align-items:stretch; }
  .mk .adc button { flex:1; font-size:11px; font-weight:700; border:1px solid var(--line); border-radius:4px;
               background:#131722; color:var(--text); padding:3px 4px; cursor:pointer; }
  .mk .adc button.adcP.on { background:rgba(38,166,154,0.15); border-color:var(--up); color:var(--up); }
  .mk .adc button.adcS.on { background:rgba(239,83,80,0.15); border-color:var(--down); color:var(--down); }
  .mk .adc select { flex:1.6; font-size:11px; background:#131722; color:var(--text); border:1px solid var(--line);
...
<div id="wrap">
  ...
  <div id="sidebar">
    ...
    <div id="revealbar">
      ...
      <span id="annCount" style="font-size:10px;color:#26a69a;white-space:nowrap;margin-top:2px"></span>
    </div>
...
el("sideBtn").onclick=()=>{ sidebarCollapsed=!sidebarCollapsed; el("sidebar").classList.toggle("collapsed", sidebarCollapsed); el("sideBtn").textContent = sidebarCollapsed ? "▶" : "◀";
  try{ localStorage.setItem("gbpusd_settings", JSON.stringify({lockRR, magnet:magnetMode, isolate:isolateSel, sidebarCollapsed, slAnchor:slAnchorMode})); }catch(e){}
  setTimeout(resize, 200);   // FIX P4: dupa transition-ul CSS (.18s) — W se recalculeaza pe noul layout
  toast(sidebarCollapsed ? "Panou ascuns — ◀ revine" : "Panou afișat");
  requestRender(); };
...
let blind = true, cutoff = view.start+view.count-1;
let lookaheadPlacing=false;   // v7.47: mod plasare lookahead (butonul Lookahead) — click pune linia O DATA, apoi click-ul revine la normal   // LOOKAHEAD OFF by default — viitorul e gri, dezvălui cu +10/+60 (butonul 🔒)
let sessionId = Date.now();
let markers = [], drawings = [];
let hist=[], histIdx=-1;   // UNDO/REDO stack — declarat INAINTE de save() (altfel TDZ crash la load)
let mkSeq = 0, drwSeq = 0, xhair=null, drag=null, dlgOpen=false, hoverMk=null, rafPending=false;
// ---- TAG-URI (v7.42): desenele au etichete permanente — trade = #NN · desene = litera tip + cifre ----
...
function ensureTags(){   // backfill: desenele vechi fara tag primesc eticheta (ca TV — fiecare obiect are ID)
...
let lastSl=6.5, lastTp=6.5, lastFc=60, lastExp=11;   // v7.51: expiry default 11 candele — pus cu arrow pe NW candle, expiry-ul se termina bine in dreapta   // "same size": ultimele setari SL/TP/forecast/EXPIRY devin default la urmatorul marker
// lastExp = expiry entry (bare): dacă entry-ul nu e atins în N candele de la candela următoare => poziția EXPIRED
try{ const s=JSON.parse(localStorage.getItem("gbpusd_last")||"null"); if(s){ if(s.sl>0)lastSl=s.sl; if(s.tp>0)lastTp=s.tp; if(s.fc>=20)lastFc=s.fc; if(s.exp>=1)lastExp=s.exp;
  // LOOKAHEAD DEFAULT: dacă ai salvat până unde ai dezvăluit, pornim fix acolo (nu mai dai +60 de fiecare dată)
...
let dlgCtrlInsert=false;   // v7.42c: ultimul mousedown = Ctrl+click pt tag — mouseup-ul NU inchide dialogul
...
function buildTF(tf){
  if(tf==="M1") return DATA_M1.length ? DATA_M1 : DATA_BASE;   // M1 real (94k bare) dacă există
...
function switchTF(tf){
  ...
  markers.forEach((m,i)=>{ if(mkTs[i]!=null){ const b=findBarFloor(mkTs[i]); if(b>=0) m.bar=b; } });   // v7.54: floor (bara exactă poate lipsi pe alt TF)
  ...
  yLock=null; zoomAnim=null; yRangeCache=null;   // FIX: DATA s-a schimbat — scala veche e stale
  document.querySelectorAll("#tfbar button").forEach(b=>b.classList.toggle("on", b.dataset.tf===tf));
  el("shead").querySelector("b").textContent="GBPUSD.pro · "+tf+" · 01 Mai – 31 Iul 2026";
...
let dlgTarget=null;   // v7.43: obiectul editat in dialog (marker sau drawing) — pt auto-save la click-out
let dlgNoteBase="";    // valoarea notei la deschidere — salvez doar daca s-a modificat
...
const anchored=true; // right-bar-stays MEREU (ca TradingView) — toggle-ul a fost ELIMINAT (v7.32): zoomul orizontal se limitează la ultima candelă vizibilă din dreapta, nu „fuge" în dreapta la zoom out · Ctrl+scroll = pointer (handled direct in wheel handler)
let yOff=0;          // pan VERTICAL liber (unitati de pret) — chart-ul se misca oriunde
...
// FIX TDZ CRITIC: indicators/omarParams declarate INAINTE de load — altfel accesul din blocul de load
...
function __restoreState(){   // RULEAZA DUPA incarcarea datelor (DATA.length > 0 — altfel filtrul sterge tot)
if(storageOK){
  try{
    ...
    if(o){
      ...
      if(recovered && (markers.length||drawings.length)){   // persistăm profilul corectat (M15 + tf) — ca să nu mai detecteze data viitoare
        try{ const prof=JSON.stringify(normalizeToM15({markers,drawings,indicators,omarParams}));
          localStorage.setItem("gbpusd_profile", prof);
          localStorage.setItem("gbpusd_profile_bak", prof);
          toast("🔧 Profil recuperat: era salvat pe "+srcTF+" → remapat pe M15 (marker-e + desene)");
        }catch(e){} }
    }
  }catch(e){}
}
// FIX v7.31: re-numerotez ghost-urile secvential 1..N la load (ID-urile 600k/700k din versiunile vechi dispar — raman #1..#N curate)
{ let gseq=1;
...
let magnetMode="hard";       // 🧲 MAGNET (ca TradingView): "off" | "soft" | "hard" — lipește prețul de OHLC/wick/SL/TP/desene. Ctrl ținut = hard temporar
let isolateSel=true;         // 🔍 IZOLARE la selectare (default ON): când selectezi o poziție, pe chart se vede DOAR ea (restul dispar — fără clutter). Comutabil din click dreapta.
let slAnchorMode=false;      // 📌 SL ANCHOR: când tragi SL-ul pe o lumânare cu Ctrl, salvează referința în nota poziției
try{ const st=JSON.parse(localStorage.getItem("gbpusd_settings")||"null"); if(st){ lockRR=!!st.lockRR; if(st.magnet) magnetMode=st.magnet; if(st.isolate!==undefined) isolateSel=!!st.isolate; if(st.sidebarCollapsed!==undefined) sidebarCollapsed=!!st.sidebarCollapsed; if(st.slAnchor!==undefined) slAnchorMode=!!st.slAnchor; } }catch(e){}   // persistat: revine la ultima valoare
if(sidebarCollapsed){ el("sidebar").classList.add("collapsed"); el("sideBtn").textContent="▶"; }   // FIX P4: aplica starea persistata la load
renderList();   // afiseaza sidebar-ul imediat (fix: nu aparea dupa refresh)

function resize(){ DPR=window.devicePixelRatio||1;
  const r=cv.getBoundingClientRect();
  W=Math.max(r.width||0, 300);   // FIX P1: doar masurarea REALA (r.width) + minim de siguranta — fallback-ul fix (innerWidth-350) facea W artificial mai mare decat canvas-ul afisat
...
function fmtT(ts){ return new Date(ts*1000).toISOString().slice(0,16).replace("T"," "); }   // UTC — folosit in EXPORT (analiza pe UTC)
...
function yrange(){
  if(drag && drag.mode==="pan" && drag.yr){   // FIX WARP v3: scala INGHETATA la mousedown (inclusiv vzoom), doar SHIFT-ata cu delta yOff — fara re-aplicare vzoom (cauza warp la zoom)
...
// 🧲 MAGNET (ca TradingView): lipește prețul de candela sub cursor (O/H/L/C = open/high/low/close),
// plus liniile SL/TP ale pozițiilor vizibile și punctele desenelor. "soft" = doar dacă ești în raza de
// 8px de țintă; "hard" = mereu pe cel mai apropiat (Ctrl ținut = hard temporar).
...
function snapPrice(bar, price, y){
  ...
  // OPTIMIZARE LAG: scala se calculeaza O SINGURA DATA (yrange()/baseRange() e scump — loop peste toate
  ...
  // FARA ALOCARI: nu construim array de candidati — urmarim direct cel mai apropiat (best/bestDy).
  ...
  // 2) liniile SL/TP + entry ale pozițiilor din setul activ (doar cele din viewport, ca TV)
  ...
  if(hard) return best;                           // HARD: mereu pe cel mai apropiat țintă
  return bestDy<=MAGNET_PX ? best : price;        // SOFT: doar dacă ești aproape de țintă
}
let ctrlPressedMagnet=false;   // Ctrl ținut în timpul drag-ului = magnet HARD temporar (ca TradingView)

// ============================================================================
// OMAR NOWICK INDICATOR — port 1:1 din Pine Script v6 ("omarnowick")
...
// ============================================================================
const PIVOT_LEN=5;   // fallback; valoarea reala vine din omarParams.pivotLen (setări)
...
function drawOmar(){
  ...
  // --- No-Wick triangles (pe TOATE datele — candelele sunt istorice; cutoff ascunde doar WIN/LOSS) ---
  if(omarParams.showNoWick) for(const s of omar.nowick){
    ...
    ctx.fillStyle=s.bull?nwBull:nwBear;   // NEON: verde/pink — se deosebesc instant de pozitiile tale
    ctx.beginPath();
    if(s.bull){ ctx.moveTo(x-6,y+14); ctx.lineTo(x+6,y+14); ctx.lineTo(x,y+5); }   // 14px sub low — NU atinge candela
    else      { ctx.moveTo(x-6,y-14); ctx.lineTo(x+6,y-14); ctx.lineTo(x,y-5); }   // 14px deasupra high — NU atinge candela
    ctx.fill();
  }
  // --- Pivots HH/HL/LH/LL (confirmate — pe TOATE datele) ---
...
const IND_TYPES={
  ...
  ema:{ name:"EMA", desc:"Exponential Moving Average", defaults:{periods:"20,50,200"},
        fields:[["periods","Perioade (virgulă)","text"]] },
  ...
  bb:{ name:"Bollinger", desc:"Bollinger Bands", defaults:{period:20, mult:2},
       fields:[["period","Perioadă","num"],["mult","Deviații","num"]] },
  vwap:{ name:"VWAP", desc:"Volume Weighted Avg Price (zi)", defaults:{},
         fields:[] },
};
indicators=[{type:"omar", on:true, params:Object.assign({}, IND_TYPES.omar.defaults)}];   // default Omar — dacă localStorage are alți indicatori, load-ul îi înlocuiește
let indCache={};
...
function renderIndList(){
  const l=el("indlist");
  if(!indicators.length){ l.innerHTML='<div style="font-size:11px;color:#787b86;padding:4px 8px">Niciun indicator. ＋ Adaugă.</div>'; return; }
  l.innerHTML=indicators.map((ind,i)=>{
    ...
    return '<div class="ind'+(ind.on?" on":"")+'">'+
      ...
      '<button data-s="'+i+'" title="Setări">⚙</button>'+
      '<button data-x="'+i+'" title="Șterge">✕</button></div>'+
...
el("addInd").onclick=()=>{
  ...
  pk.innerHTML=Object.keys(IND_TYPES).filter(t=>!indicators.some(i=>i.type===t)).map(t=>
    '<button data-t="'+t+'">'+IND_TYPES[t].name+' <small>— '+IND_TYPES[t].desc+'</small></button>').join("");
...
function render(){
  ...
  const yP=p=>((hi-p)/(hi-lo)*H);           // O(1) local — NU yAtPrice (evita re-apele yrange = LAG la zoom mare)
  // grid orizontal
  ...
  const cw=candleW();
  // lumanari — BATCHED (Path2D per culoare): fara beginPath/stroke per candela = fara LAG la zoom mare
  const wUp=new Path2D(), wDn=new Path2D();
  const bUp=new Path2D(), bDn=new Path2D();
  const lastDraw=blind?Math.min(cutoff, end-1):end-1;   // v7.44: candelele din DREAPTA liniei lookahead DISPAR COMPLET (ca TV — viitorul nu se vede deloc, nici cu opacitate)
  ...
  // linia de revelare (v7.46: evidenta — trage-o cu mouse-ul pe orice candela, schimbari instant)
  ...
  // marker-e
  // 🔍 IZOLARE (default ON): când o poziție e selectată, desenăm DOAR ea pe chart — restul dispar (fără clutter).
  // drawMarker are deja highlight pe selMk (contur galben + badge R:R), deci izolatul se vede clar.
  // ATENȚIE: izolarea desenează selMk DOAR dacă e din setul vizibil (ghost===nwVisible) — altfel drawMarker
  // face early-return și marker-ul DISPARE complet (fix: în cazul ăsta desenăm tot setul, nu nimic).
  ...
  // indicatori (Omar + orice adăugat din catalog) — mereu vizibili, ca TradingView
  ...
  // marquee box (Ctrl+drag) — selectie multipla ca TradingView
  ...
  // backtest vizual: rulează SL/TP pe barele viitoare pentru fiecare marker
  if(btOn && btOverlay) drawBacktest();
  // axa pret — IN price bar (dreapta, full-height, ca TradingView)
  ...
  if(xhair){
    ...
    ctx.fillText("C "+fmtP(c[4])+"    · "+fmtP(xhair.price), 16, 56);
  ...
  const rng=d0.toISOString().slice(0,10)+" → "+d1.toISOString().slice(0,10);
  const bd=blind?("blind · până la "+fmtTRO(DATA[Math.min(cutoff,end-1)][0]).slice(0,10)):"tot chart-ul";
  el("status").innerHTML="<b>"+rng+"</b> · "+view.count+" bare · "+bd+" · "+markers.length+" marker-e · "+drawings.length+" desene · <span title=\"Datele sunt UTC; afișat în ora României (UTC+3 vară / +2 iarnă). Analiza/export = UTC.\">🕐 RO</span>";
  yRangeCache=null;
  el("debug").textContent="canvas "+W+"x"+H+" (backing "+cv.width+"x"+cv.height+") · lumanari desenate "+drawOps+" · blind "+(blind?"ON":"OFF")+" · view "+view.start+".."+(end-1)+" · vzoom "+vzoom.toFixed(2)+"x · js:ok";
  drawOps=0;
}
...
function drawMarker(m){
  ...
  // ghost (AI): CULOARE MONO pe direcție — LONG = tot verde, SHORT = tot roșu (separare clară)
  const gcol = m.ghost ? (up ? "#26a69a" : "#ef5350") : null;   // pastram pt banda (subtila), dar liniile SL/TP sunt STANDARD ca la pozitiile tale
  // exit-ul real (primul SL/TP atins pe barele viitoare) — ca TradingView
  const bt=(m.sl!=null && m.tp!=null) ? btMapCur(btRun(m)) : null;   // v7.54: rezultatul (M1) e mapat pe TF-ul afișat
  // FILL LA TOUCH: trail-ul + liniile SL/TP încep de la PRIMA candelă care atinge entry-ul (fillBar),
  // nu de la bara markerului (care poate fi înainte ca prețul să ajungă la entry)
  ...
  const fcBars = (m.fc>0)?m.fc:60;                              // forecast length (bare) — "same size", ca TV
  ...
  // MINIM 2 CANDELE vizual: dacă intrarea și ieșirea sunt pe ACEEAȘI candelă (39% din bot trades), banda e 1px = invizibilă.
  // Extindem vizual banda + liniile SL/TP la cel puțin 2 candele (trail-ul/punctul de exit rămân la poziția reală).
  const xEndVis = Math.max(xEnd, x0 + 2*bw());
  const colEx = bt ? (bt.res==="WIN" ? "#26a69a" : bt.res==="LOSS" ? "#ef5350" : "#f5c542") : null;
  // banda pozitiei — IDENTICA pentru ghost si ale tale: SL side ROSU, TP side VERDE (ca TV)
  ...
  // SL (rosu) si TP (verde) — LINII DRAGGABLE, din prima candelă care atinge entry pana la exit
  ...
  if(noFill){   // nu s-a atins entry-ul — trade-ul nu s-a executat (doar sageata + eticheta, fara bandă/trail)
    ctx.fillStyle="#787b86"; ctx.font="bold 9px Segoe UI, Arial"; ctx.textAlign="left";
    ctx.fillText("no fill", x+6, y+(up?-10:20));
  }
  if(expired){   // EXPIRY: entry-ul nu a fost atins în exp candele — semnul stă „în aer" lângă candela de start
    // (NU atinge prima candelă ex. 21:45), cu lungime orizontală ~4-5 candele + mic emoji = ratat/expired
    const bwp=bw();
    const sx=x+1.5*bwp;                    // decalat la dreapta: nu atinge candela
    const sw=Math.max(34, 4.5*bwp);        // ~4-5 candele lungime orizontală
    ...
    ctx.fillText("⏳", sx+sw+4, y+4);      // mic emoji: poziția a expirat
    ctx.font="bold 8px Segoe UI, Arial";
    ctx.fillText("expired", sx+2, y+(up?-7:13));
  }
  // linia de intrare + sageata — din PRIMA candela care atinge entry pana la exit (culori standard; neon-ul e doar pe indicatorul Omar)
  // EXPIRED: linia de intrare NU se desenează (semnul plutitor ține locul — nu vrem linia să atingă candela)
  if(!expired){
  ...
  ctx.fill();
  if(m.id==null) m.id=++mkSeq;   // marker-e vechi fara id → generez (sigur, nu crapa)
  try{  // badge #ID pe chart — izolat: daca un marker are date proaste, NU opreste restul chart-ului
  ...
  if(m.note){ ctx.fillStyle="rgba(19,23,34,0.88)";
    ...
    ctx.fillText(m.note.length>34?m.note.slice(0,34)+"…":m.note, nx+6, y-12); }
  // handles SL/TP + highlight DOAR pe SELECTAT (hover-ul schimba doar cursorul — curat, ca TV)
  if(selSet.length>1 && selSet.some(it=>it.kind==="m" && it.ref===m) && m!==selMk){
    // multi-select: contur galben subtil (fara handles/badge — doar semnalizare)
  ...
  if(m===selMk){
    ...
    ctx.globalAlpha=1;
    // badge R:R + pips (mic, FARA cerculete) — LONG: deasupra liniei TP (sus) · SHORT: sub linia TP (jos)
    if(m.sl!=null && m.tp!=null){
      const rr=m.sl>0?(m.tp/m.sl):0;
      // WIN/LOSS/OPEN vizibil la selectare — culori clare (verde/roșu/galben), ca TradingView
      ...
      const badge="R:R "+rr.toFixed(2)+" · SL "+m.sl+"p · TP "+m.tp+"p"+(lockRR?" · 🔒":"");
      ...
      const by = up ? tyLine-25 : tyLine+6;                 // LONG: deasupra · SHORT: sub
      ...
      // badge WIN/LOSS (mic, deasupra badge-ului R:R) — se vede clar ce a fost trade-ul
      if(resTxt){
...
// preview LONG/SHORT in spatele dialogului — live, inainte de Salveaza
function drawMarkerPreview(p){
  const b=p.bar, x=xAtBar(b), y=yAtPrice(p.price);
  if(b<view.start || b>=viewEnd()) return;
  const up=p.dir==="LONG";
  // exit-ul real (primul SL/TP atins) — live, ca TradingView
  const bt=(p.sl!=null && p.tp!=null) ? btMapCur(btRun(p)) : null;   // v7.54: mapat pe TF-ul afișat
  const fcBars = (p.fc>0)?p.fc:60;                              // forecast length (bare) — "same size"
  ...
  // EXPIRED preview: semnul plutitor (segment + emoji) ca pe chart — se vede în spatele dialogului
  ...
  ctx.fill();
  ctx.restore();
  const lab=(up?"▲ LONG":"▼ SHORT")+" — SL "+(p.sl!=null?p.sl:"-")+"p · TP "+(p.tp!=null?p.tp:"-")+"p";
...
// Shift+drag — dreptunghi de masura (ca TradingView): pips + bare + timp
function drawMeasure(m){
  ...
  const lab=pips.toFixed(1)+" pips · "+bars+" bare · "+fmtDur(Math.round(bars*15));
  ctx.font="10px Segoe UI, Arial"; ctx.textAlign="left";
  const w=ctx.measureText(lab).width+12;
...
// ============================================================================
// BACKTEST VIZUAL — rulează SL/TP pe barele viitoare pentru fiecare marker
// Anti-lookahead: în blind mode, rezultatul apare doar dacă exit-bar <= cutoff
// ============================================================================
let btOn=false;              // toggle
let btOverlay=true;          // overlay liniilor backtest (eye on/off)
let btCache={ json:"", stats:null };   // panou actualizat doar când se schimbă
// cache btRun: fingerprint pe (id,bar,price,sl,tp,cutoff,TF) — rezultatul se invalideaza automat la orice modificare
let btCacheMap=new Map();
function btRun(m){
  // v7.56: rezultatul se calculează pe datele de 1 MINUT (adevărul fin), indiferent de view-ul afișat.
  // Fereastra de expirare e aliniată la candelele TF-ului de plasare: fill-ul începe la următoarea
  // candelă de plasare (ts + tfMin), fereastra = exp × minutele TF-ului (11 M15 = [ts+15, ts+180)).
  // Deci 11 candele se comportă ca pe 15m, dar fill/SL/TP sunt calculate pe M1 — precizia reală.
  const ev = DATA_M1.length ? DATA_M1 : DATA_BASE;
  // ancora de timp a poziției: m.ts (fix la plasare) — fallback pentru marker-e vechi: bara curentă
  ...
  const expBars = m.exp===0 ? 0 : (m.exp>0?m.exp:lastExp);   // 0 explicit = fără expiry; undefined = default salvat (lastExp)
  const expMin = expBars * tfMin;
  // start: următoarea candelă de plasare (sau chiar ancora cu "Entry aici"); capăt: start + fereastra
  ...
  // limite: până la capătul datelor REALE — candelele sunt istorice, deci evaluăm peste tot.
  // cutoff blochează DOAR plasarea marker-elor în blind, NU rezultatul real al backtest-ului.
  ...
  // FILL LA TOUCH: prima candelă (după bara markerului) care atinge efectiv prețul entry
  // (bar-touch fill — ca MT5): candela e atinsă dacă entry-ul e în intervalul [low, high].
  // De acolo începe trail-ul + evaluarea SL/TP. Dacă nicio candelă nu atinge → NOFILL (nu s-a executat).
  // EXPIRY: marker-ele fără exp folosesc default-ul salvat (lastExp). Dacă entry-ul nu e atins în
  // fereastra de expirare (ex. 11 candele M15 = 165 min) de la următoarea candelă => EXPIRED.
  let fillBar=null;
  ...
  if(res===null && fillBar<=lim) res={res:"OPEN", bar:lim, price:ev[lim][4]};   // încă în desfășurare
...
// rezultatul btRun e în coordonatele datelor de EVALUARE (M1) — îl mapăm pe TF-ul afișat pt desenare
function btMapCur(r){
...
let btStatsCache={ fp:-1, json:"", stats:null };   // OPTIMIZARE LAG: btStats e apelat la fiecare render (drawBacktest) — cache pe fingerprint (markers+nwVisible), doar marker-ele schimbate invalideaza
function btStats(){
  // fingerprint ieftin: lungime + suma id/bar/sl/tp + starea NW — se schimba DOAR cand marker-ele se schimba
  ...
  const rows=[];
  for(const m of markers){
    if(m.ghost !== nwVisible) continue;                  // switch NW: doar setul activ în statistici
  ...
  const st={ n:rows.length, wins:wins.length, losses:losses.length, opens:opens.length, noFills:noFills.length, expired:expireds.length,
           ...
           totR, pf: (pf===Infinity?"∞":pf.toFixed(2)), winPips, lossPips };
  btStatsCache={ fp, json:JSON.stringify(st), stats:st };
  return st;
}
function drawBacktest(){
  // liniile entry → exit pentru fiecare marker rezolvat
  for(const m of markers){
  ...
  // panou statistici (doar când se schimbă) — JSON.stringify e scump, cache-ul il tine deja serializat
  const st=btStats();
  const j=btStatsCache.json;
  if(j!==btCache.json){
    btCache.json=j; btCache.stats=st;
    const p=el("btPanel");
    p.innerHTML='<b>🧪 Backtest</b>'+
      ...
      '<div class="r"><span>Σ R</span><span>'+(st.totR>0?"+":"")+st.totR+'R</span></div>'+
...
el("btBtn").onclick=()=>{ btOn=!btOn;
  ...
  if(btOn){
    // Backtest = vezi direct trade-urile WIN/LOSS (verde/roșu), fără să desenezi tu fiecare poziție:
    // 1) nimic desenat → generez automat backtest-ul bot (BOT8_SPEC) pe tot istoricul + îl arăt pe chart
    // 2) setul vizibil e gol, dar celălalt are trade-uri → comut pe el (ghost AI ↔ ale tale)
    if(markers.length===0){ autoBotTrades();
      if(!nwVisible){ nwVisible=true; el("nwBtn").classList.add("on");
        el("nwBtn").title = "Ghost ON: vezi DOAR backtest trades — apasă din nou ca să revin ale tale"; } }
    else if(!markers.some(m=>m.ghost===nwVisible)){ nwVisible=!nwVisible;
      el("nwBtn").classList.toggle("on", nwVisible);
      el("nwBtn").title = nwVisible ? "Ghost ON: vezi DOAR backtest trades — apasă din nou ca să revin ale tale" : "Ghost OFF: vezi doar trade-urile tale — apasă ca să vezi backtest trades"; }
    btCache.json=""; btStats();
  }
  requestRender(); };
el("btEye").onclick=()=>{ btOverlay=!btOverlay;
  el("btEye").classList.toggle("off", !btOverlay);
  el("btEye").textContent = btOverlay ? "👁" : "🚫";
  requestRender(); toast(btOverlay?"Liniile backtest vizibile":"Liniile backtest ascunse"); };

// Ghost markers — semnalele No-Wick din ZONA VIZIBILĂ (nu doar până la cutoff!).
// Candelele sunt date istorice (vizibile și în zona gri); cutoff ascunde DOAR WIN/LOSS (btRun se oprește la el).
function autoNWGhosts(){
  const from = Math.max(0, view.start-60);                       // buffer mic spre stânga
  ...
  for(let i=from; i<=to; i++){
    ...
    if(Math.abs(bodyPips)<3) continue;                      // body minim 3p (relaxat — vezi mai multe trade-uri)
    const bull = (o===l) && cl>o;                            // NW bullish: open==low (Pine-pure, tol 0)
    const bear = (o===h) && cl<o;                            // NW bearish: open==high
    if(!bull && !bear) continue;
    // toate sesiunile (fără filtru 09-17) — vezi mai multe semnale; filtrezi tu cu PASS/SKIP
    if(markers.some(m=>m.bar===i)) continue;                 // nu dubla marker-ele existente
    const dir = bull?"LONG":"SHORT";
    // SL config reală: la extremul candelei NW + buffer, DAR minim 5p (sub spread nu se poate — NW pur are O==L, deci low==open)
    ...
    markers.push({ id:++mkSeq, bar:i, price:o, dir, ghost:true, tf:TF, ts:DATA[i][0],   // v7.54: evaluare pe M1, ancorat aici
      note:"👻 NW "+(dir==="LONG"?"bull":"bear")+" · body "+bodyPips.toFixed(1)+"p · SL "+sl+"p / TP "+tp+"p · PASS/SKIP? DE CE?",
      sl, tp, fc:60, created:Date.now(),
      seen_through:(blind && i<=cutoff) ? cutoff : null });  // zona gri: rezultatul e necunoscut
    n++;
  }
  if(n>0){ save(); requestRender(); toast("👻 "+n+" ghost markers puse — click pe fiecare: PASS/SKIP + DE CE + SL/TP real"); }
  else toast("👻 Niciun semnal NW în zona vizibilă — pan/zoom spre altă zonă sau dezvălui mai mult");
}

// 🤖 Bot — backtest automat: rulează config-ul bot-ului (BOT8_SPEC) pe TOT istoricul și pune trade-urile ca ghost markers
function autoBotTrades(){
  computeOmar();   // asigură nowick + pivots actualizate
  // IDEMPOTENT: ștergem ghost-urile bot generate anterior (nota începe cu "🤖 Bot") — re-apăsarea NU dublează trade-urile
  for(let i=markers.length-1;i>=0;i--){
    if(markers[i].ghost && markers[i].note && markers[i].note.startsWith("🤖 Bot")) markers.splice(i,1);
  }
  const MIN_BODY=5, SESS0=240, SESS1=1170, BREATH=1.10, MIN_SL=5, MAX_SL=15, RR=1.0, EXPIRY_BARS=10;
  const made=[];
  let activeUntil=-1;   // o singură poziție pe rând (decizia #11)
  for(const s of omar.nowick){
    const i=s.bar;
    if(i<60) continue;                                  // warm-up indicatori
    if(i<=activeUntil) continue;                        // poziția anterioară încă activă (expiry)
    const c=DATA[i]; if(!c) continue;
    ...
    // pivot-wick activ = ultimul pivot de structură opus (pt LONG: ultimul low; pt SHORT: ultimul high)
    let pivot=null;
    ...
    made.push({ id:++mkSeq, bar:i, price:c[1], dir:up?"LONG":"SHORT", ghost:true, tf:TF, ts:DATA[i][0],   // v7.54: evaluare pe M1, ancorat aici
      note:"🤖 Bot "+(up?"bull":"bear")+" · SL "+sl+"p / TP "+tp+"p · pivot "+pivot.label+" @"+pivot.price.toFixed(DIG),
      sl, tp, fc:EXPIRY_BARS, created:Date.now(), seen_through:null });
    activeUntil=i+EXPIRY_BARS;                          // expiry 10 bare (decizia #12)
  }
  markers.push(...made);
  // ID-uri SIMPLE 1..N: renumerotez ghost-urile bot secvential (in ordinea barelor) — nu 600k, nu ++mkSeq
  let seq=1;
  const bots=markers.filter(m=>m.ghost && m.note && m.note.startsWith("🤖 Bot")).sort((a,b)=>a.bar-b.bar);
  for(const b of bots) b.id=seq++;
  btOn=true; btOverlay=true;   // backtest vizual ON (liniile WIN/LOSS) — dar chart-ul ghost-urilor e controlat de 👻 NW
  ...
  toast("🤖 Bot: "+made.length+" trade-uri generate (config BOT8_SPEC) pe tot istoricul");
}
// Contor live: câte semnale NW sunt în zona vizibilă (fără să apeși 👻)
let nwCountCache=-1;
function updateNwCount(){
  ...
  if(n!==nwCountCache){ nwCountCache=n;
    const e=el("nwCount");
    e.textContent = "· "+n+" semnale NW vizibile";
    e.title = n+" semnale No-Wick (O==L/O==H, body≥3p) în zona vizibilă — 👻 NW le pune ca ghost markers";
  }
}
el("nwBtn").onclick=()=>{ nwVisible=!nwVisible;                 // SWITCH: ON = doar backtest trades (ghost), OFF = doar ale tale (nu șterge)
  el("nwBtn").classList.toggle("on", nwVisible);
  el("nwBtn").title = nwVisible ? "Ghost ON: vezi DOAR backtest trades — apasă din nou ca să revin ale tale" : "Ghost OFF: vezi doar trade-urile tale — apasă ca să vezi backtest trades";
  if(nwVisible){ autoNWGhosts(); btOn=true; btOverlay=true;     // pune ce lipsește + vezi INSTANT trade-urile WIN/LOSS
...
el("lockRRBtn").onclick=()=>{ lockRR=!lockRR;
  ...
  el("lockRRBtn").title = lockRR ? "🔒 R:R fix ON: SL și TP se mută împreună (același raport) — apasă ca să deblochezi" : "🔒 R:R fix OFF: SL/TP independente — apasă ca să le lege (același raport la mutare)";
  try{ localStorage.setItem("gbpusd_settings", JSON.stringify({lockRR, magnet:magnetMode, isolate:isolateSel, sidebarCollapsed, slAnchor:slAnchorMode})); }catch(e){}   // persistat — revine la ultima valoare la refresh
  toast(lockRR ? "🔒 R:R fix ON — SL și TP se mută proporțional" : "🔓 R:R fix OFF — SL/TP independente");
  requestRender(); };
el("lockRRBtn").classList.toggle("on", lockRR);   // starea persistata se reflecta pe buton la load
el("magBtn").onclick=()=>{   // 🧲 Magnet: ciclare off -> soft -> hard (ca TradingView)
  magnetMode = magnetMode==="off" ? "soft" : magnetMode==="soft" ? "hard" : "off";
  el("magBtn").classList.toggle("on", magnetMode!=="off");
  const label = magnetMode==="hard" ? "HARD (mereu pe cel mai apropiat)" : magnetMode==="soft" ? "SOFT (doar când ești aproape)" : "OFF";
  el("magBtn").title = "🧲 Magnet "+label+". Lipește prețul de wick-uri, OHLC, SL/TP și desene. Ține Ctrl în timpul drag-ului = Hard temporar.";
  el("magBtn").querySelector("span").textContent = "🧲 Magnet: "+label.split(" ")[0];
  try{ localStorage.setItem("gbpusd_settings", JSON.stringify({lockRR, magnet:magnetMode, isolate:isolateSel, sidebarCollapsed, slAnchor:slAnchorMode})); }catch(e){}
  toast("🧲 Magnet "+label);
  requestRender(); };
el("magBtn").classList.toggle("on", magnetMode!=="off");
try{ el("magBtn").querySelector("span").textContent = "🧲 Magnet: "+(magnetMode==="hard"?"HARD":magnetMode==="soft"?"SOFT":"OFF"); }catch(e){}
el("botBtn").onclick=()=>{
  // 🤖 Bot = DOAR switch în object tree: AI trades ↔ my trades (2 stări). NU atinge chart-ul.
  botActivated=true;   // prima apăsare: ghost-urile bot devin vizibile în listă (până acum erau ascunse la load)
  const hasGhost = markers.some(m=>m.ghost);  if(!hasGhost){ autoBotTrades(); treeView="ai"; el("botBtn").classList.add("on"); renderList(); return; }   // prima apăsare: generează + arată AI
  treeView = treeView==="ai" ? "mine" : "ai";   // toggle simplu: AI ↔ mine
  el("botBtn").classList.toggle("on", treeView==="ai");
  renderList();
  toast("Object tree: "+(treeView==="ai"?"AI trades (backtest)":"trade-urile tale"));
};

// 📋 Finalizează sesiunea — raport complet pe deciziile tale (după backtest)
function finalizeSession(){
  ...
  const ghost = rows.filter(x=>x.m.ghost);                 // ghost: le-ai păstrat → TAKE implicit
  const bySess = {};
  for(const x of take){
  ...
  function stat(arr){
    ...
    return {n:arr.length,w,l,o,wr:w+l?(w/(w+l))*100:0,totR,pf:pf===Infinity?"∞":pf.toFixed(2),wp,lp};
  }
  ...
  const p=el("btPanel");
  p.innerHTML='<b>📋 Raport sesiune</b>'+
    ...
    '<div class="r"><span>Σ R</span><span>'+(stT.totR>0?"+":"")+stT.totR+'R</span></div>'+
    '<div class="r"><span>Profit factor</span><span>'+stT.pf+'</span></div>'+
    '<div class="r" style="border-top:1px solid var(--line);margin-top:4px;padding-top:4px"><span>Ghost păstrate</span><span>'+stG.n+'</span></div>'+
    ...
    '<div class="r" style="border-top:1px solid var(--line);margin-top:4px;padding-top:4px"><span>Total (toate)</span><span>'+stAll.n+' · WR '+stAll.wr.toFixed(0)+'%</span></div>';
  el("btPanel").style.display="block";
  btOn=true; el("btBtn").classList.add("on");
  requestRender();
  toast("📋 Sesiune finalizată — raport în panoul din stânga");
}
el("finBtn").onclick=()=>finalizeSession();

function markerPartAt(x, y){
  const tol=5;   // toleranta linii SL/TP (pe toata lungimea — drag direct, ca inainte)
  for(const m of [...markers].reverse()){
    if(m.ghost !== nwVisible) continue;                  // switch NW: ghost ascuns = NEATINS (hitbox mort, ca sa nu blocheze cursorul)
    if(m.bar<view.start || m.bar>=viewEnd()) continue;
    const ex=xAtBar(m.bar);
    // zona X: de la entry pana la capatul box-ului — SINCron cu vizualul (drawMarker):
    // dacă trade-ul s-a închis devreme (WIN/LOSS), vizualul se oprește la exit, NU la forecast (fc=60)
    ...
    // 1) LINIA SL/TP — pe TOATA lungimea pentru pozitiile tale; la ghost AI doar langa sageata (ex+40)
    //    (altfel hover-ul/comentariul lung de pe ghost prind slider-ul peste tot — hitbox urias)
    ...
    // 2) linia de entry — DOAR pe sageata insasi (cerc mic in jurul varfului), nu pe toata lungimea benzii
    if(Math.hypot(x-ex, y-yAtPrice(m.price))<16) return {m, part:"entry"};
    // 3) BODY: interiorul benzii INTRE linii (fill) = muta TOT — pe TOATA lungimea box-ului vizual (pana la xEnd)
...
// box de comentariu pe desene (linie/zonă/hline) — în dreapta, ca TV
function drawNoteBox(ctx, x, y, text){
  ...
  ctx.fillText(text.length>36?text.slice(0,36)+"…":text, nx+6, y+13);
}
// SHIFT = constrângere la 45° (ca TradingView): punctul (x,y) față de ancora (ax,ay) se lipește de
// unghiurile 0° (orizontal), 90° (vertical), 45°/135° (diagonale) — în PIXELI pe ecran.
function constrain45(x, y, ax, ay){
  const dx=x-ax, dy=y-ay;
  const adx=Math.abs(dx), ady=Math.abs(dy);
  // diagonala la 45°: |dx| == |dy|; altfel axa dominantă (ca TV)
  if(adx<=ady*0.5){ return [ax, y]; }        // aproape vertical → vertical pur
  if(ady<=adx*0.5){ return [x, ay]; }        // aproape orizontal → orizontal pur
  const d=Math.min(adx, ady);                // bandă diagonală → 45° exact (semnul păstrat)
...
function drawDrawing(d, selected, isDraft){
  const pt=(p,i)=>{ const x=xAtBar(p.bar), y=yAtPrice(p.price);
    ...
    return [x,y]; };
  ctx.strokeStyle=isDraft?"#ffa726":(selected?"#81d4fa":COL.draw);
  ...
  if(d.type==="pen" && pts.length>=2){
    ...
    // HANDLES la capete (ca TradingView): cerculete mici, vizibile DOAR pe selectat — tragi de ele = schimbi unghi/lungime
    ...
    if(d.text){ ctx.fillStyle="rgba(19,23,34,0.88)";
      ...
      ctx.fillStyle=COL.text; ctx.font="10px Segoe UI, Arial";
      ctx.fillText(d.text.length>40?d.text.slice(0,40)+"…":d.text, a[0]+16, a[1]-12); } }
...
function hitDrawing(x, y, tol){
  // capetele desenului SELECTAT au PRIORITATE (ca TradingView): handles mici la capete/colțuri
...
// badge WIN/LOSS/OPEN/no fill pentru lista din dreapta — culoare + text mic, ca pe chart
function resBadge(m){
  try{
...
function renderList(){
  const l=el("list");
  let html='';
  if(!markers.length && !drawings.length){ l.innerHTML='<div style="font-size:12px;color:#787b86;padding:8px">Nimic încă. Click pe chart = marker; alege o unealtă de desen din toolbar.</div>'; return; }
  if(drawings.length){
    html+='<div class="sechead">Desene<span class="cnt">'+drawings.length+'</span></div>';
    drawings.forEach((d,i)=>{
      const lab={line:"📐 Linie",pen:"✏️ Pen",rect:"🟦 Zonă",hline:"➖ H-Linie",arrow:"💬 Notă"}[d.type]||d.type;
      const a=d.pts[0], b=d.pts[1]||d.pts[0];
      html+='<div class="drw'+(sel===d?" sel":"")+'" data-d="'+i+'"><b>'+(d.tag?'<span style="color:#4a6fd4;font-weight:800">'+d.tag+'</span> ':'')+lab+'</b> · '+
        fmtTRO(DATA[a.bar][0])+' → '+fmtTRO(DATA[b.bar][0])+'<br>'+
        '<span style="color:#787b86">x:'+DATA[a.bar][0]+'..'+DATA[b.bar][0]+' · y:'+a.price.toFixed(5)+'..'+b.price.toFixed(5)+'</span>'+
        '<div style="margin-top:4px"><button data-dd="'+i+'">🗑</button></div></div>';
    });
  }
  if(markers.length){
    const ghosts=markers.filter(m=>m.ghost), mine=markers.filter(m=>!m.ghost);
    // treeView: 'all' = ambele secțiuni · 'ai' = doar ghost AI · 'mine' = doar ale tale (butonul 🤖 Bot = switch în object tree, NU atinge chart-ul)
    if(ghosts.length && treeView!=="mine" && botActivated){
      html+='<div class="sechead" style="color:#f5c542;border-top-color:#f5c542;background:rgba(245,197,66,0.08)">👻 Ghost AI — Backtest<span class="cnt">'+ghosts.length+'</span></div>';
    ...
    if(mine.length && treeView!=="ai"){
      ...
      [...mine].sort((a,b)=>a.bar-b.bar).forEach(m=>{
        ...
        html+='<div class="mk '+(m.dir==="SHORT"?"sell":"")+selCls+'" data-mid="'+m.id+'"><div class="top"><span class="tag '+(m.dir==="LONG"?"buy":"sell")+'">#'+m.id+' '+m.dir+
          ...
          (m.sl!=null?'<div class="meta">SL '+m.sl+'p · TP '+(m.tp!=null?m.tp:"-")+'p · R:R '+(m.sl>0?(m.tp/m.sl).toFixed(2):"-")+'</div>':'')+
          '<textarea class="inote" data-mid="'+m.id+'" placeholder="Comentariu (de ce)…" rows="2">'+(m.note?escapeHtml(m.note):"")+'</textarea>'+
          '<div class="meta">grup: <select class="igrp" data-mid="'+m.id+'">'+
            '<option value="">—</option>'+[1,2,3,4,5].map(g=>'<option value="'+g+'"'+(m.group==g?" selected":"")+'>Grup '+g+'</option>').join('')+
          '</select> · seen până la '+seen+(m.blind?' · blind':' · tot chart-ul')+'</div>'+
          '<div class="acts"><button data-a="edit" data-mid="'+m.id+'">✎ detalii</button><button data-a="del" data-mid="'+m.id+'">🗑 șterge</button></div></div>';
      });
    }
  }
  l.innerHTML=html;
...
// Sincronizează lista din dreapta cu selecția de pe chart: evidențiază rândul poziției selectate
// + face scroll la el (ca să nu o cauți după ID pe un chart aglomerat).
function syncListSel(){
  try{   // FIX: dacă lista nu are încă rânduri (sau altceva crapă), NU opri click-ul/dblclick-ul pe chart
  const l=el("list");
  // curăță evidențierea veche, apoi aplică pe cea activă (selMk sau multi-select)
...
// ---- COPY / PASTE (Ctrl+C / Ctrl+V) — copiază obiectele selectate de pe chart (ca TradingView) ----
let clipData=null;   // {markers:[], drawings:[]} — copia din Ctrl+C
function copySelection(){
  ...
  if(!mk.length && !dr.length){ toast("Selectează întâi ce vrei să copiezi (click pe poziție/desen sau Ctrl+drag box)"); return; }
  // deep copy + id-uri curate (paste-ul generează altele noi)
  clipData={ markers: mk.map(m=>({...m})), drawings: dr.map(d=>({...d, pts:d.pts.map(p=>({...p}))})) };
  toast("📋 Copiat: "+mk.length+" poziții, "+dr.length+" desene — Ctrl+V să lipești");
}
function pasteClipboard(){
  if(!clipData || (!clipData.markers.length && !clipData.drawings.length)){ toast("Clipboard gol — mai întâi Ctrl+C pe ceva selectat"); return; }
  ...
  // offset: 5 bare la dreapta + puțin preț (ca să vezi duplicatul lângă original)
  const dBar=5, dPrice=2*PIP;
  for(const m of clipData.markers){
    const nb=Math.max(0,Math.min(mx,m.bar+dBar));
    markers.push({ ...m, id:++mkSeq, bar:nb, ts:(DATA[nb]?DATA[nb][0]:m.ts), price:m.price+dPrice, created:Date.now() });   // v7.54: ts urmează bara lipită
  }
  ...
  save(); requestRender();
  toast("📋 Lipit: "+clipData.markers.length+" poziții, "+clipData.drawings.length+" desene (offset +5 bare)");
}

// ---- UNDO / REDO (Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y + butoane ↩️ ↪️) ----
// (hist declarat sus, inainte de save — vezi 'let hist' langa markers)
function pushHist(){
...
function reveal(n){ if(!blind) return; cutoff=Math.min(DATA.length-1, cutoff+n); yLock=null; yRangeCache=null;
  ...
  requestRender(); }
// ---- PLAY: dezvăluie candela următoare; Space = toggle, ține apăsat = accelerare ----
...
function toggleBlind(){
  if(blind){ blind=false; cutoff=DATA.length-1; toggleBlindUI();
    ...
    toast("Blind dezactivat — vezi tot chart-ul"); }
  else { blind=true; cutoff=Math.min(DATA.length-1, view.start+view.count-1);
    toggleBlindUI();
    el("blindBtn").title="Blind ON: ascunde viitorul — dezvălui cu +10/+60";
    toast("Lookahead ON — candelele viitoare ascunse"); }
  yLock=null; yRangeCache=null;   // FIX: cutoff-ul s-a schimbat — scala veche e stale
  requestRender(); }
...
function openDlg(bar, price, m){
  ...
  const exp=(m&&m.exp>0)?m.exp:lastExp;   // expiry entry (bare) — dacă nu e atins în N candele => expired
  dlgTarget=m||null; dlgNoteBase=(m&&m.note)||"";   // v7.43: target + valoare initiala pt auto-save
  dlg.innerHTML='<h4>'+(m?"Editează":"Marchează")+' poziție — '+fmtTRO(DATA[bar][0])+' @ '+fmtP(price)+'</h4>'+
    '<div class="row"><label>Direcție</label><div class="dirbtns">'+
    '<button class="dir buy '+(up?"on":"")+'" data-dir="LONG">▲ LONG</button>'+
    '<button class="dir sell '+(!up?"on":"")+'" data-dir="SHORT">▼ SHORT</button></div></div>'+
    ...
    '<div class="row"><label>Forecast (bare) — lățimea box-ului</label>'+
    ...
    '<div class="row"><label>Expiry entry (bare) — dacă prețul nu atinge entry-ul în N candele ⇒ expired</label>'+
    '<div style="display:flex;align-items:center;gap:6px">'+
    '<input id="exp" type="range" min="0" max="60" step="1" value="'+exp+'">'+
    '<span id="expVal" style="min-width:34px;text-align:right">'+(exp>0?exp:"∞")+'</span></div></div>'+
    '<div class="row"><label>DE CE ai lua trade-ul? (ce vezi pe chart)</label>'+
    '<textarea id="note" placeholder="ex: trend clar, NW la pullback, CHoCH recent, sesiune bună…">'+(m?escapeHtml(m.note):"")+'</textarea></div>'+
    '<div class="acts"><button class="cancel">Anulează</button><button class="save">Salvează</button></div>';
  dlg.style.display="block"; dlgOpen=true; dlgOpenedAt=Date.now();
  // CENTRAT (ca TradingView) — dialogul nu mai acoperă locul de pe chart pe care îl descrii
  ...
  function updPrev(){ const slv=parseFloat(dlg.querySelector("#sl").value), tpv=parseFloat(dlg.querySelector("#tp").value);
    ...
    el("expVal").textContent=expv>0?expv:"∞";
  ...
  dlg.querySelector(".save").onclick=()=>{
    ...
    if(m){ const i=markers.indexOf(m); markers[i]=mk;
      // FIX: marker-ul editat e ÎNLOCUIT cu obiect nou (mk) — selMk/selSet trebuie sa pointeze la NOUL obiect,
    ...
    toast("Marker salvat (SL "+mk.sl+"p / TP "+mk.tp+"p / forecast "+fcv+"b)"+(note?" — "+note.slice(0,40):"")); };
...
function openArrowDlg(d, isNew){
  ...
  dlg.innerHTML='<h4>💬 Notă — '+fmtTRO(DATA[p.bar][0])+' @ '+fmtP(p.price)+'</h4>'+
    '<div class="row"><label>DE CE? Subiectivitatea ta, în cuvintele tale (ce vezi aici):</label>'+
    '<textarea id="note" placeholder="ex: trendul s-a stricat aici, nivelul a rezistat de 3 ori, aici aș fi intrat dar era lângă știri…">'+escapeHtml(d.text||"")+'</textarea></div>'+
    '<div class="acts"><button class="cancel">Anulează</button><button class="save">Salvează</button></div>';
  dlg.style.display="block"; dlgOpen=true; dlgOpenedAt=Date.now();
  // CENTRAT (ca TradingView) — dialogul nu mai acoperă locul de pe chart pe care îl descrii
  const r=box.getBoundingClientRect();
  ...
  dlg.querySelector(".save").onclick=()=>{ d.text=dlg.querySelector("#note").value.trim();
    ...
    save(); closeDlg(); requestRender(); toast("Notă salvată"); };
  dlg.querySelector(".cancel").onclick=()=>{ closeDlg(); requestRender(); };  // CANCEL NU STERGE NIMIC — desenul rămâne (Delete = ștergere)
  dlg.querySelector("#note").focus();
}

function closeDlg(saveIfDirty){   // v7.43: saveIfDirty=true => click-out — salveaza nota daca s-a modificat (nu o pierzi la click gresit)
...
// DIALOG MUTABIL (ca TradingView): trage de ORICARE din dialog ca să-l muți — doar câmpurile unde scrii nu se trage de pe ele
let dlgDrag=null;   // {dx,dy} offset mouse↔dialog (null = nu se trage)
el("dlg").addEventListener("mousedown", ev=>{
  if(!dlgOpen) return;
  const t=ev.target;
  // câmpuri editabile / interactive: cursor normal, drag NU pornește de pe ele
...
function toggleBlindUI(){
  ...
  el("blindBtn").title = blind ? "Lookahead ON: viitorul e ascuns (gri) — dezvălui cu +10/+60 sau Play" : "Lookahead OFF: vezi tot chart-ul — apasă ca să ascunzi viitorul";
...
function resetView(){   // FIT CONTENT (ca TradingView) — buton nou + tasta A + dblclick pe pricebar → un singur loc
  view.start=0; view.count=250; vzoom=1; yOff=0;
  yLock=null; yRangeCache=null; zoomAnim=null; vzoomAnim=null;
  requestRender();
  toast("Reset view — 250 bare, zoom 100%");
}
function setTool(t){
...
el("delD").onclick=()=>{ if(sel){ drawings.splice(drawings.indexOf(sel),1); sel=null; save(); requestRender(); toast("Desen șters"); } };

// ⭐ FAVORITES plutitoare (ca TradingView): PILL mic ⭐ = grab handle (tragi de el oriunde pe chart).
// Click pe ⭐ = deschide popup cu uneltele favorite (nu se lungește pe chart). Click dreapta pe o
// unealtă din toolrail = adaugi/scoti din favorite.
let favTools=[];
...
const FAV_NAMES={ pointer:"Select", long:"LONG", short:"SHORT", line:"Linie", pen:"Pen", rect:"Zonă", hline:"H-Linie", arrow:"Notă" };
function favPopupOpen(){ return false; }
let favPlaced=false;   // pill-ul a primit o pozitie initiala (left/top) — fara asta depindea de 'right' si se intindea
...
function renderFavBar(){
  const fb=el("favbar");
  // TOATE uneltele favorite vizibile direct in bara (fara popup) — click = unealta, click dreapta = scoate
  ...
  for(const t of favTools){
    ...
    b.dataset.t=t; b.title=FAV_NAMES[t]+" (click = alege · click dreapta = scoate din favorite)";
    b.innerHTML='<svg><use href="#'+FAV_ICONS[t]+'"/></svg>';
    b.onclick=()=>setTool(t);
    b.oncontextmenu=ev=>{ ev.preventDefault(); favTools=favTools.filter(x=>x!==t); saveFavs(); renderFavBar(); toast("⭐ scos din favorite: "+FAV_NAMES[t]); };
    fb.appendChild(b);
  }
...
document.querySelectorAll("#toolrail [data-t]").forEach(b=>b.addEventListener("contextmenu", ev=>{
  ev.preventDefault();
  const t=b.dataset.t;
  if(favTools.includes(t)){ favTools=favTools.filter(x=>x!==t); toast("⭐ scos din favorite: "+FAV_NAMES[t]); }
  else { favTools.push(t); toast("⭐ adăugat la favorite: "+FAV_NAMES[t]+" — click pe ⭐ să-l folosești"); }
  saveFavs(); renderFavBar();
}));
// drag pe PILL (⭐ = grab handle) = muti pill-ul oriunde pe chart; popup-ul urmează
let favDrag=null;
...
window.addEventListener("mousemove", ev=>{
  ...
  x=Math.max(2, Math.min(x, box.width-fb.offsetWidth-2));   // doar left/top — fara right, pill-ul nu se intinde
...
// ---- MENIU CLICK DREAPTA (ca TradingView) — înlocuiește meniul default al browser-ului pe chart ----
...
function showCtxMenu(x,y){ ctxMenu.style.display="block";
  ...
  // starea lockRR vizibila in meniu (✓ / ✗)
  const lr=el("ctxLockRR");
  lr.classList.toggle("on", lockRR);
  lr.textContent = lockRR ? "🔒 R:R fix — ON ✓" : "🔒 R:R fix — OFF";
  // starea IZOLARE vizibila in meniu (✓ / ✗)
  const iso=el("ctxIsolate");
  iso.classList.toggle("on", isolateSel);
  iso.textContent = isolateSel ? "🔍 Izolează la selectare — ON ✓" : "🔍 Izolează la selectare — OFF";
  // starea SL Anchor vizibila in meniu (✓ / ✗)
  const slA=el("ctxSlAnchor");
  slA.classList.toggle("on", slAnchorMode);
  slA.textContent = slAnchorMode ? "📌 SL Anchor — ON ✓" : "📌 SL Anchor — OFF";
  // v7.50: setari pozitie — vizibile doar cand e selectata o pozitie (expiry on/off + candele + entry)
...
cv.addEventListener("contextmenu", ev=>{ ev.preventDefault();
  // click dreapta pe o poziție = o selectează automat (ca TradingView) — preset-ul R:R se aplică pe ea
...
ctxMenu.querySelectorAll(".ci").forEach(ci=>ci.onclick=ev=>{
  ...
  if(cmd==="pointer") setTool(TOOLS.POINTER);
    ...
    toast(isolateSel ? "🔍 Izolare ON — la selectare se vede doar poziția respectivă" : "🔍 Izolare OFF — se văd toate pozițiile");
    requestRender(); }
  else if(cmd==="slanchor"){ slAnchorMode=!slAnchorMode;
    try{ localStorage.setItem("gbpusd_settings", JSON.stringify({lockRR, magnet:magnetMode, isolate:isolateSel, sidebarCollapsed, slAnchor:slAnchorMode})); }catch(e){}
    showCtxMenu(ev.clientX||100, ev.clientY||100);
    toast(slAnchorMode ? "📌 SL Anchor ON — trage SL-ul pe o lumânare cu Ctrl ca să ancorezi referința" : "📌 SL Anchor OFF");
    requestRender(); }
  else if(cmd==="bt") el("btBtn").onclick();
  else if(cmd==="export") el("exp").onclick();
});
// v7.50: setari pozitie din click-dreapta — expiry on/off + nr candele + entry aici/urmatoarea
...
el("ctxExpOn").onchange=()=>{ const t=ctxSelectedPos(); if(!t) return;
  ...
  save(); requestRender(); toast("Expiry "+(t.exp>0?("ON — "+t.exp+" candele"):"OFF")); };
...
el("ctxEntryNext").onclick=()=>{ const t=ctxSelectedPos(); if(!t) return;
  t.entryMode="next"; save(); requestRender(); toast("Entry la candela următoare"); };
// preset R:R: aplica raportul pe pozitia selectata (TP = SL × rr, pastrand directia)
ctxMenu.querySelectorAll(".rrpreset").forEach(btn=>btn.onclick=ev=>{
  ev.stopPropagation();
  const rr=parseFloat(btn.dataset.rr);
  const t=selMk || (selSet.length && selSet.find(it=>it.kind==="m") ? selSet.find(it=>it.kind==="m").ref : null);
  if(!t || t.sl==null || t.sl<=0){ toast("Selectează întâi o poziție cu SL"); return; }
  t.tp=Math.round(t.sl*rr*10)/10;
  save(); requestRender(); hideCtxMenu();
  toast("Poziție #"+(t.id??"")+" → R:R "+rr+" (TP "+t.tp+"p)");
});
...
cv.addEventListener("mousemove", ev=>{
  ...
  // 🧲 MAGNET: lipește prețul de OHLC/wick/SL/TP/desene — Ctrl ținut (inclusiv în drag) = hard temporar
  ...
  if(drag){
    ...
    if(!drag.moved && dm>6){
      ...
      // — fara asta, primii 6px se aplicau dintr-o data = chart-ul sarea sub mouse la inceputul drag-ului
      if(drag.mode==="pan"){
        ...
        // FIX PAN MORT: dupa re-baseline, mișcarea din ACEST eveniment trebuie aplicata (nu zero).
    ...
    if(drag.mode==="posdraft" && posDraft){
      // muti box-ul liber pe chart (slide across) — bar + price urmeaza cursorul
      const mx=blind?cutoff:DATA.length-1;
      ...
      if(drag.mp.part==="body"){
        ...
        if(inMulti){  // muta TOATE pozitiile din selSet impreuna
          ...
          for(const it of selSet){
            if(it.kind==="m"){ it.ref.bar=Math.max(0,Math.min(mx,it.ref.bar+dBar)); it.ref.price+=dPrice; it.ref.ts=DATA[it.ref.bar]?DATA[it.ref.bar][0]:it.ref.ts; }   // v7.54: ancora urmează bara nouă
            else it.ref.pts.forEach(p=>{ p.bar=Math.max(0,Math.min(mx,p.bar+dBar)); p.price+=dPrice; });
          }
        } else {
          m.bar=Math.max(0, Math.min(mx, bar)); m.price=price; m.ts=DATA[m.bar]?DATA[m.bar][0]:m.ts;   // v7.54: ancora urmează bara nouă
        }
        requestRender(); return; }
      ...
      if(pips>=0.5 && pips<=100){
        ...
        if(okDir){ const v=Math.round(pips*10)/10;
          if(drag.mp.part==="sl"){
            ...
            m.sl=v;
            if(lockRR && rr!=null) m.tp=Math.round(v*rr*10)/10;   // 🔒 muta si TP-ul proporțional
          } else {
            const rr=(m.tp>0 && m.sl!=null) ? m.tp/m.sl : null;
            m.tp=v;
            if(lockRR && rr!=null && m.sl>0) m.sl=Math.round(v/rr*10)/10;   // 🔒 muta si SL-ul proporțional
          }
        }
      }
      }
    ...
    if(drag.mode==="ptmove" && drag.d){
      // drag pe CAPĂTUL liniei/rect-ului: muta DOAR acel punct (unghi/lungime, ca TradingView)
      const d=drag.d, pi=drag.pi, mx=blind?cutoff:DATA.length-1;
      let pBar=bar, pPrice=price;
      if(ev.shiftKey){   // SHIFT = constrangere 45° fata de celalalt capat (ca desenare)
    ...
    if(drag.mode==="move" && sel){
      // SHIFT = constrângere la mutare pe o singură axă (ca TradingView): orizontal SAU vertical (dominantă)
      ...
      const mx=blind?cutoff:DATA.length-1;
      // daca desenul e in multi-select → muta TOATE elementele din selSet impreuna
      if(selSet.length>1 && selSet.includes(sel)){
        for(const it of selSet){
          ...
          else { it.ref.bar=Math.max(0,Math.min(mx,it.ref.bar+dBar)); it.ref.price+=dPrice; it.ref.ts=DATA[it.ref.bar]?DATA[it.ref.bar][0]:it.ref.ts; }   // v7.54: ancora urmează bara nouă
        }
      } else {
    ...
    if(drag.mode==="pan" && drag.moved){
      ...
      // FIX WARP v3: shift vertical cu scala COMPLETA inghetata (include vzoom+pad) — imaginea ramane LIPITA de mouse
      yOff=drag.yOff0 + ddy*(drag.yr.hi-drag.yr.lo)/H;
      yRangeCache=null;   // view-ul s-a schimbat — scala veche e stale; se recalculeaza pe view-ul curent
      // OPTIMIZARE LAG: pan-ul = doar mutarea imaginii. Skip hover/magnet/hit-test (inutile in timpul
      // pan-ului) — fara asta, fiecare mousemove rula snapPrice+markerPartAt+hitDrawing (sute de apeluri
  ...
  if(draft && (tool===TOOLS.LINE||tool===TOOLS.RECT)){
    // SHIFT = constrângere la 45° (ca TradingView): Linie → unghiuri 0/45/90°; Rect → pătrat
  ...
  xhair={ bar, price };
  // hover: doar CURSORUL se schimba (fara handles galbene) — grab pe body/mijloc, ns-resize pe SL/TP/entry
...
cv.addEventListener("mousedown", ev=>{
  ...
  const bar=barAtX(x);
  let price=priceAtY(y);
  // 🧲 MAGNET la plasare: primul click (LONG/SHORT/line/rect/hline/arrow) prinde direct wick-ul/OHLC/SL/TP
  ctrlPressedMagnet=!!ev.ctrlKey;
  if(magnetMode!=="off" || ctrlPressedMagnet) price=snapPrice(bar, price, y);
  // v7.44: DRAG pe linia de lookahead (cutoff) — o muti pe orice candela; viitorul dispare dupa ea
  ...
  // v7.49: Shift+click NU mai pune bara de lookahead (Shift e pt toolurile de masurare/constrangere) — foloseste butonul 📏 Bara sau drag pe linie
  // v7.47: click pe gol DOAR in mod plasare lookahead (butonul Lookahead) — pune linia O DATA, apoi click-ul revine la normal
  if(blind && lookaheadPlacing && tool===TOOLS.POINTER && !ev.ctrlKey && x>=60 && x<=W-52
    ...
    toast("Lookahead setat la candela "+(bar+1)+" — click-ul a revenit la normal"); return;
  }
  if(ev.shiftKey && tool===TOOLS.POINTER){   // Shift+drag = masura (ca TradingView) — DOAR cu unealta Select; cu unealta de desen, Shift = constrangere 45°
    drag={ mode:"measure", downX:x, downY:y, bar, price, moved:false };
    measure={ b0:bar, p0:price, b1:bar, p1:price };
    requestRender(); return; }
  // NOTĂ: Ctrl+drag (selbox multi-select) e verificat MAI JOS, DUPĂ hit-test-ul de obiecte —
  // ca în TradingView, Ctrl+click pe un marker/desen = DRAG CU MAGNET (precizie), nu selbox.
  // Ctrl+drag pe GOL = selbox (rămâne).
  if(tool!==TOOLS.POINTER){
    ...
    if(tool===TOOLS.LONG || tool===TOOLS.SHORT){
      // UN singur click plaseaza box-ul si revine la Select — click-ul stanga ramane liber dupa
      ...
      setTool(TOOLS.POINTER);   // click-ul e liber de acum — doar tragi box-ul / click pe el = dialog
      toast(dir+" plasat — trage box-ul unde vrei; click pe box = salvezi cu comentariu; Esc = anulezi");
      requestRender(); return;
    }
    if(tool===TOOLS.HLINE){
      drawings.push({ id:++drwSeq, type:"hline", pts:[{bar:bb, price}], tag:nextDrawTag("hline") });
      save(); requestRender(); toast("H-Linie salvată — revenit la Select"); setTool(TOOLS.POINTER); return; }
    if(tool===TOOLS.ARROW){
      const d={ id:++drwSeq, type:"arrow", pts:[{bar:bb, price}], text:"" };
      // arrow-ul NU e inca in drawings — se adauga DOAR la Save; Cancel/close = dispare fara urma
    ...
    if(tool===TOOLS.PEN){
      ...
      requestRender(); return; }
    // LINE / RECT: CLICK-CLICK (ca TradingView) — click 1 pune primul punct, muti mouse-ul (preview), click 2 finalizeaza
    if(draft && (tool===TOOLS.LINE||tool===TOOLS.RECT)){
      if(Date.now()-draft.t0<250 && Math.hypot(x-draft.sx, y-draft.sy)<8) return;  // bounce (mouse/touchpad)
      // click 2: finalizeaza — pune linia/zona unde e preview-ul (SHIFT = constrangere 45° ca la preview)
      let p1={bar:bb, price};
      if(ev.shiftKey && draft.pts[0]){
        const p0=draft.pts[0];
      ...
      if(ok){ const nd2={ id:++drwSeq, type:draft.type, pts:[p0,p1] }; nd2.tag=nextDrawTag(draft.type); drawings.push(nd2);
        toast("Desen salvat — revenit la Select"); }
      else { toast("Prea mic — click pe alt punct pentru Linie/Zonă"); }
      draft=null; setTool(TOOLS.POINTER); save(); requestRender();
      return; }
    draft={ type:tool, pts:[{bar:bb, price}], t0:Date.now(), sx:x, sy:y, sx2:x, sy2:y };
    toast("Click 1 pus — mută mouse-ul, click din nou ca să pui linia (Esc = anulezi)");
    requestRender();
    return; }
  ...
  if(posDraft){   // draft-ul e prioritar: tragi direct pe el (linii SL/TP + interior)
    ...
    if(x>=ex-12 && x<=xEnd+12){
      ...
      // entry — DOAR pe sageata insasi (cerc mic in jurul varfului), nu pe toata lungimea benzii
      if(Math.hypot(x-ex, y-yAtPrice(posDraft.price))<16){ drag={mode:"posdraft", mp:{part:"entry"}, downX:x, downY:y, moved:false}; return; }
      // body: interiorul benzii INTRE linii → muta tot (pe tot box-ul vizual)
      if(sy!=null||ty!=null){
  ...
  if(mp){ 
    ...
    syncListSel();   // evidențiază + scroll la poziție în lista din dreapta (nu o mai cauți după ID)
    requestRender(); return; }
  // select desen / pan
  ...
  if(h){
    ...
    // capăt de linie/rect prins (_pt setat de hitDrawing) => drag DOAR pe acel punct (schimbi unghi/lungime, ca TV)
    ...
    return; }
  // Ctrl+drag pe GOL = box de selectie multipla (ca TradingView) — verificat DOPĂ hit-test-ul,
  // ca Ctrl+click pe un marker/desen să fie drag cu magnet, nu selbox
  if(ev.ctrlKey){
  ...
  // FIX TELEPORTARE (zoom orizontal + click): yLock îngheață scala la wheel — dacă îl resetam ÎNAINTE
  // de drag.yr, scala sărea de la cea desenată (yLock) la baseRange recalculat => imaginea "teleportată".
  // Scala trebuie înghețată pe ce e DESENAT ACUM, apoi yLock poate fi resetat.
  // v7.33: drag.yr = scala ABSOLUTA DESENATA (yrange aplica yLock+vzoom+yOff) — un singur apel, fara
  // ramura if(yLock) separată care dubla yOff la pan după freeze (bug: pan vertical → drag pricebar → teleport).
  drag.yr=yrange();
  yLock=null;
  vzoomAnim=null; zoomAnim=null;   // FIX: oprește animațiile de zoom la mousedown — dacă vzoom-ul se schimba
  // în timpul pan-ului, drag.yr (înghețat) rămânea în urmă => imaginea sărea la fiecare pas de animație
  if(selMk){ selMk=null; syncListSel(); requestRender(); }   // click pe gol = deselectezi pozitia (stergi si evidențierea din listă)
});
cv.addEventListener("dblclick", ev=>{
  if(dlgOpen) return;
  const r=cv.getBoundingClientRect(); const x=ev.clientX-r.left, y=ev.clientY-r.top;
  // FIX: hit-test pe box-ul VIZUAL complet (markerPartAt prinde SL/TP/entry/body pe toată lățimea vizibilă),
  // nu doar pe bara exactă a marker-ului — altfel dblclick pe linia SL/TP sau pe corpul box-ului nu deschidea dialogul
  const mp=markerPartAt(x,y);
  if(mp) openDlg(mp.m.bar, mp.m.price, mp.m);
  else {
    // dblclick pe gol cu IZOLARE ON = dezizolează (revine tot) — ca să poți edita alt trade/bot fără să cauți comutatorul
...
window.addEventListener("mouseup", ev=>{
  ...
  if(dlgDrag){ dlgDrag=null; return; }     // sfârșit drag dialog — nu închide
  // LINE/RECT: finalizarea se face la CLICK 2 (in mousedown) — mouseup-ul nu mai deseneaza nimic
  if(dlgOpen){
    if(Date.now()-dlgOpenedAt<300) return;    // mouseup-ul care a deschis dialogul (Nota) — ignorat
    const t=ev.target;
    if(t && el("dlg").contains(t)) return;   // click in dialog — butoanele merg
    if(dlgCtrlInsert){ dlgCtrlInsert=false; return; }   // v7.42c: era Ctrl+click pt tag — NU inchide dialogul
    closeDlg(true); return;                    // v7.43: click in afara — AUTO-SAVE nota daca s-a modificat (nu o pierzi)
  }
  if(!drag) return;
  ...
  if(drag.mode==="posdraft" && posDraft){
    ...
    if(!wasMoved && Math.hypot(ddx,ddy)<=6){
      // click simplu pe box-ul plasat = SALVEAZA DIRECT (fara dialog — click-ul stanga nu mai marcheaza)
    ...
    markers.push(mk); save(); requestRender();
      toast(mk.dir+" salvat (SL "+mk.sl+"p / TP "+mk.tp+"p) — dublu-click sau ✎ pt. comentariu");
    } else {
      requestRender();
    }
    return; }
  if(drag.mode==="pospart"){ drag=null; requestRender(); return; }
  if(drag.mode==="ptmove"){ drag=null; save(); requestRender(); return; }   // capăt de linie/rect: salvează poziția nouă
  if(drag.mode==="mkpart"){
    // 📌 SL ANCHOR: când slAnchorMode e ON și s-a tras SL-ul, salvează referința la lumânare în notă
    if(slAnchorMode && drag.mp.part==="sl" && drag.moved){
      ...
      if(c){
        ...
        const anchor=`📌 SL ancorat pe bara #${bar} (${bestL} ${fmtP(levels[bestL])})`;
        m.note=m.note ? m.note.replace(/📌 SL ancorat.*/,"")+" · "+anchor : anchor;
      }
    }
  ...
  if(drag.mode==="pen"){
    if(draft && draft.type==="pen" && draft.pts.length>=2){
      ...
      toast("Pen salvat ("+draft.pts.length+" puncte) — revenit la Select");
    } else if(draft && draft.pts.length<2){ toast("Prea mic — desenează ținând apăsat"); }
    draft=null; setTool(TOOLS.POINTER); save(); requestRender();
    return; }
  if(drag.mode==="move"){ drag=null; return; }
  // click simplu pe fundal (fara drag) → deselecteaza tot multi-selectul
  if(selSet.length>1 && !drag.moved){ selSet=[]; sel=null; selMk=null; save(); }
  drag=null;   // click stanga = doar select/pan — NU marcheaza pozitii (fara dialog)
});
cv.addEventListener("wheel", ev=>{
  ev.preventDefault();                 // suprimă zoom-ul de pagină al browser-ului, inclusiv Ctrl+wheel (ca TV)
  if(dlgOpen) return;                  // nu misca chart-ul sub dialog
  vzoomAnim=null; zoomAnim=null;       // FIX TELEPORTARE: oprește animațiile de zoom vertical la scroll pe chart —
  // altfel animația (rămasă de la scroll pe pricebar) continua pe view-ul NOU și scala sare sus/jos
  ...
  // ZOOM CA TRADINGVIEW (v7.32): Ctrl+scroll = zoom la POINTER (bara sub cursor fixa) · scroll simplu = zoom la
  // right bar (ULTIMA candela vizibila din dreapta ramane fixa) — modelul lightweight-charts right_bar_stays_on_scroll.
...
window.addEventListener("keydown", ev=>{
  ...
  // UNDO/REDO — prioritar, chiar si peste textarea (dar acolo lasa browser-ul sa faca undo de text)
  ...
  // COPY / PASTE (Ctrl+C / Ctrl+V) — obiectele selectate de pe chart (marker-e + desene), ca TradingView
  ...
  // SAVE (Ctrl+S) — persistează tot (marker-e, desene, indicatori, setări) ca "profil"
  if(ev.ctrlKey && !ev.shiftKey && (ev.key==="s"||ev.key==="S") && !inField){ ev.preventDefault(); save(); toast("💾 Salvat (Ctrl+S) — tot ce ai pe chart e persistat"); return; }
  // TASTE UNELTE (ca TradingView) — doar cand nu scrii intr-un camp si nu e dialog deschis
  if(!inField && !dlgOpen){
    const k=ev.key.toLowerCase();
    const toolMap={ v:"pointer", b:"long", s:"short", l:"line", p:"pen", r:"rect", h:"hline", n:"arrow" };
    if(toolMap[k]){ ev.preventDefault(); setTool(toolMap[k]); toast("Unealtă: "+toolMap[k]); return; }
    // timeframes: 1=M15, 2=H1, 3=H4, 4=D1
    const tfMap={ "1":"M15", "2":"H1", "3":"H4", "4":"D1" };
    if(tfMap[k]){ ev.preventDefault(); switchTF(tfMap[k]); return; }
    if(k==="m"){ ev.preventDefault(); toast("Măsură: ține Shift + drag pe chart"); return; }
    if(k==="a"){ ev.preventDefault(); resetView(); return; }
    if(k==="f"){ ev.preventDefault(); toast("Filtre: vezi panoul 👻 NW + PASS/SKIP"); return; }
  }
  ...
  if(!inField){
    ...
    // Play: Space/Ctrl+Space = toggle; ține apăsat = accelerare (repeat keydown)
    if(ev.code==="Space"){
      ev.preventDefault();
  ...
  if(ev.key==="Delete" && !dlgOpen && !inField){
    // Delete pe multi-select → șterge TOATE elementele selectate (desene + poziții)
    if(selSet.length>1){
      for(const it of selSet){
      ...
      toast(selSet.length+" elemente șterse"); selSet=[]; sel=null; selMk=null; save(); requestRender(); return;
    }
    if(selMk){ const i=markers.indexOf(selMk); if(i>=0){ markers.splice(i,1); toast("Marker șters"); } selMk=null; selSet=[]; save(); requestRender(); return; }
...
// ---- zoom smooth orizontal (rAF) — ancorat la MOUSE, ca TradingView ----
const ZOOM_MS=150;
function clampCount(c){ return Math.max(4, Math.min(DATA.length, Math.round(c))); }  // FIX P3: zoom-in pana la 4 bare (era 30) — ca TradingView/lightweight-charts; max zoom out = tot dataset-ul, zero void
function anchorFrac(){
  // ON = right bar (frac 1 — right_bar_stays_on_scroll, default TradingView)
...
function stepZoom(){
  ...
  if(c!==a.lastCount){                     // skip frame fara schimbare (fara flicker la plateaus)
    ...
    const ms=Math.max(0, DATA.length-c);   // FIX: maxStart cu count-ul NOU (nu cel vechi) — fara derapaj la zoom
    view.start=Math.max(0, Math.min(ms, Math.round(target)));
    view.count=c; a.lastCount=c;
    yRangeCache=null;                        // FIX: view-ul s-a schimbat — scala veche e stale
  }
  requestRender();
  if(t<1) requestAnimationFrame(stepZoom); else zoomAnim=null;
}
// zoom INSTANT la wheel (ca TV): factor proportional cu delta, ancorat la cursor, rAF-coalesced — fara coada elastica
function wheelZoom(dy, atCursor){
  yLock=null; yRangeCache=null;   // FIX P2: zoom instant la wheel — reseteaza si el yLock (startZoom nu e apelat aici)
...
function applyVzoom(nv, anchorPrice){
  // FIX TELEPORTARE (research deep): yrange() desenează cu b=yLock (scala înghețată de zoom-ul orizontal),
  // dar aici foloseam baseRange() — fracția de ancorare era calculată pe o BAZĂ DIFERITĂ de cea afișată
  // => la primul zoom vertical după un zoom orizontal, punctul de sub cursor sărea (cel mai vizibil la zoom out).
  // Trebuie exact aceeași bază ca yrange(): yLock ?? baseRange().
  ...
  yRangeCache=null;                         // FIX: scala s-a schimbat — cache-ul vechi e stale (altfel candelele sar)
}
function startVzoom(to, anchorPrice){   // FIX: NU mai ștergem yLock aici — zoom-ul vertical lucrează corect pe scala
  // înghețată (yLock) exact ca TV (manual price scale persistă); ștergerea anticipată cauza teleportare.
...
function pbarFreezeScale(){   // FIX TELEPORTARE: orice gest pe pricebar = scala devine MANUALĂ (ca TV după price-scale
  // gesture) — yLock înghețat pe scala curentă, ca zoom-ul ORIZONTAL ulterior să NU o recalculeze (baseRange
  // pe bare noi) => fără teleportare verticală când scroll-ezi pe chart după ce ai scroll-at pe bară.
  // ROOT CAUSE v7.33 (bug raportat: pan vertical → drag pricebar → teleport sus de tot):
  // yLock trebuie să fie un SNAPSHOT PUR din baseRange() — NICIODATĂ scala absolută (vzoom+yOff incluse).
  // De ce: yrange() (linia 605) re-aplică vzoom+yOff PESTE yLock. Dacă yLock conține deja vzoom+yOff,
  // scala desenată se comprimă/deplasează dublu = teleportare (confirmat matematic: span 0.02585→0.01775
...
pbar.addEventListener("mousedown", ev=>{
  ...
  vzoomAnim=null; zoomAnim=null;   // FIX: oprește animațiile la grab — altfel drag-ul lupta cu scala in miscare
  pbarFreezeScale();                // scala devine manuală (ca TV) — zoom orizontal ulterior nu o recalculează
  const r=pbar.getBoundingClientRect();
  priceBarDrag={ y:ev.clientY-r.top, v0:vzoom, moved:false };
  ev.preventDefault();
});
// dublu-click pe bara de pret (sidebar dreapta) = reset view (ca TradingView) — te de-pierzi instant
...
// 📌 PRICEBAR LABELS: crosshair la Y-ul cursorului + SL/TP/Entry pentru poziția selectată (ca TradingView)
function updatePricebar(){
  const pb=el("pbtag");
  // crosshair price: poziționat DINAMIC la Y-ul cursorului (nu fix sus)
  if(xhair){
  ...
  // SL/TP/Entry labels: vizibile DOAR când o poziție e selectată
...
el("saveBtn").onclick=()=>{ save(); toast("💾 Profil salvat (Ctrl+S) — tot ce ai pe chart e persistat"); };
el("laPlaceBtn").onclick=()=>{   // v7.48: mod plasare bara lookahead (one-shot) — SEPARAT de switch-ul Lookahead
  if(!blind){ blind=true; toggleBlindUI(); }
  lookaheadPlacing=true; requestRender();
  toast("Mod plasare bara — click pe chart o pune (o dată), apoi revine la normal"); };
el("laDefaultBtn").onclick=()=>{
  ...
  toast("🎯 Lookahead default salvat — la restart, chart-ul pornește dezvăluit până la "+fmtTRO(ts).slice(0,10)+" "+fmtTRO(ts).slice(11,16));
};
...
el("exp").onclick=()=>{  const data={ version:5, session_id:sessionId,
  ...
  // nume cu data+ora in TIMESTAMP-UL ROMANIEI (Europe/Bucharest: vara UTC+3, iarna UTC+2) — nu ora sistemului (care poate fi UTC)
  ...
  toast("JSON exportat ("+data.markers.length+" marker-e, "+data.drawings.length+" desene) — "+a.download+" → Downloads / collect_exports.py");
};

// 📤 Export MT5 EA — generează un Expert Advisor de replay cu marker-ele tale (rulează în MT5 Tester Model=2)
el("expEA").onclick=()=>{
  const sig=markers.filter(m=>m.sl!=null && m.tp!=null && m.price>0 && DATA[m.bar]);
  if(!sig.length){ toast("Niciun marker cu SL/TP — pune poziții întâi"); return; }
  let rows='';
  for(const m of sig){
  ...
  const ea=
"//+------------------------------------------------------------------+\r\n"+
"//| ReplayEA_ChartSignals.mq5 — generează din chart (v7.27.11)       |\r\n"+
"//| Rulează în Strategy Tester: GBPUSD.pro · M15 · Model=2 (every tick) |\r\n"+
"//+------------------------------------------------------------------+\r\n"+
"#property strict\r\n"+
"input int    ServerUTCOffset = 2;   // OANDA server = GMT+2 fixed; UTC = server − offset\r\n"+
...
"void OnTick(){\r\n  if(sigCount==0) return;\r\n  long barUTC = (long)(iTime(_Symbol, PERIOD_M15, 0) - ServerUTCOffset*3600);\r\n  for(int i=0;i<sigCount;i++){\r\n    if(done[i]) continue;\r\n    if(barUTC < sigs[i].ts) continue;                  // așteaptă bara de intrare\r\n    done[i]=true;\r\n    // o singură poziție per semnal (dacă mai există una deschisă de la semnalul precedent, o închidem la intrarea următoare)\r\n    for(int p=PositionsTotal()-1;p>=0;p--){\r\n      ulong tk=PositionGetTicket(p);\r\n      if(PositionSelectByTicket(tk) && PositionGetInteger(POSITION_MAGIC)==Magic)\r\n        if(!PositionClose(tk)) Print(\"close fail \",tk);\r\n    }\r\n    long type = (sigs[i].dir==\"LONG\") ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;\r\n    MqlTradeRequest req={}; MqlTradeResult res={};\r\n    req.action=TRADE_ACTION_DEAL;\r\n    req.symbol=_Symbol;\r\n    req.volume=LotSize;\r\n    req.type=(sigs[i].dir==\"LONG\")?ORDER_TYPE_BUY:ORDER_TYPE_SELL;\r\n    req.price=(sigs[i].dir==\"LONG\")?SymbolInfoDouble(_Symbol,SYMBOL_ASK):SymbolInfoDouble(_Symbol,SYMBOL_BID);\r\n    req.sl=sigs[i].sl;\r\n    req.tp=sigs[i].tp;\r\n    req.magic=Magic;\r\n    req.comment=\"sig#\"+IntegerToString(i);\r\n    if(!OrderSend(req,res)) Print(\"order fail \",i,\" err=\",res.retcode);\r\n  }\r\n}\r\n";
  ...
  document.body.appendChild(a); a.click();
  setTimeout(()=>{ document.body.removeChild(a); URL.revokeObjectURL(a.href); }, 200);
  toast("📤 EA MT5 generat cu "+sig.length+" semnale — copiază în MQL5/Experts, compilează, rulează în Tester (Model=2)");
};
...
function findBar(ts){ let lo=0, hi=DATA.length-1;
  ...
  return null; }
// v7.53: PROFIL TF-SAFE — profilul e salvat MEREU în coordonate M15; la load, dacă un profil vechi
// e pe alt TF (ex. salvat din greșeală pe M1), îl detectăm și remapăm prin timestamp. Evaluarea
// WIN/LOSS/expired NU se schimbă — doar pozițiile marker-elor/desenelor sunt corectate.
...
// detectează TF-ul în care e salvat profilul: toate barele trebuie să încapă în datele acelui TF,
// iar marker-ele trebuie să fie la timpul real de creare (TF-ul corect minimizează |ts(bar)-created|)
function detectProfileTF(o){
  const cands=["M15","M1","H1","H4","D1"];
  let best=null, bestErr=Infinity;
  for(const tf of cands){
    if(tf==="M1" && !DATA_M1.length) continue;   // M1 nu există pe acest chart — nu e candidat
    ...
    if(maxB<0) continue;               // niciun bar în profil
    if(maxB>=d.length) continue;       // nu încape în acest TF
    let err=0, n=0;
    for(const m of (o.markers||[])){
      if(!m||!Number.isInteger(m.bar)||m.bar<0||!m.created||!d[m.bar]) continue;
      n++; err+=Math.abs(d[m.bar][0]-m.created/1000);
    }
    if(n===0){ if(!best) best=tf; continue; }   // fără marker-e cu created: primul care încape (M15 implicit)
    if(err<bestErr){ bestErr=err; best=tf; }
  }
  return best||"M15";
}
// remapează barele profilului din TF-ul sursă în DATA curentă prin timestamp + pune ts de ancoră
...
// copie de salvare cu toate barele în coordonate M15 (DATA_BASE) + ts de ancoră + tf — profilul
// persistat nu mai depinde de TF-ul afișat la momentul salvării.
...
el("impFile").onchange=ev=>{
  ...
  rd.onload=()=>{ try{
    ...
    catch(err){ toast("Import eșuat: "+err.message); } };
  rd.readAsText(f);
  ev.target.value="";
};

el("cls").onclick=()=>{
  if((markers.length||drawings.length) && confirm("Ștergi tot ("+markers.length+" marker-e, "+drawings.length+" desene)?")){ markers=[]; drawings=[]; sel=null; save(); requestRender(); toast("Totul șters"); }
};
...
html = HTML
OHLC_OUT = os.path.join(HERE, "gbpusd_ohlc_2026.json")
OHLC_JS = os.path.join(HERE, "gbpusd_ohlc_2026.js")
ohlc_obj = {"m15": rows, "m1": rows_m1, "retrace": retrace_markers}
ohlc_js = json.dumps(ohlc_obj, separators=(",", ":"), ensure_ascii=False)
with open(OHLC_OUT, "w", encoding="utf-8") as f:
    f.write(ohlc_js)
with open(OHLC_JS, "w", encoding="utf-8") as f:
    f.write("window.GBPUSD_OHLC_2026=" + ohlc_js + ";")
with open(OUT, "w", encoding="utf-8") as f:
    f.write(html)
print("scris", OUT, os.path.getsize(OUT), "bytes |", OHLC_OUT, os.path.getsize(OHLC_OUT), "bytes |", OHLC_JS, os.path.getsize(OHLC_JS), "bytes |", len(rows), "bare |", len(retrace_markers), "retrace")
